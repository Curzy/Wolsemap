...
def record_station_data(station_id, file_object) :
...
def crawl_dabang(station_id, page_number):
    ...
    station_json = json.load(reader(response))

    response.close()

    total_room = station_json['total']
    subway_line = station_json['subway']['line']
    ...
    if subway_line[0] not in filtering_line : #수도권 호선인지 검사 #방이 없는 역일 경우 넘김
        ...
        return station_json #방이 있으나 수도권 역이 아님

def averaging(station_json) :
    ...
    station_name = station_json['subway']['name']
    subway_line = station_json['subway']['line']

    while True :
        for room in station_json['rooms'] :
            #"price_info": [[1000,40]]
        ...
        has_more = station_json['has-more']
        if has_more :
            station_json = crawl_dabang(station_json['subway']['id'], station_json['next-page'])
        else :
            break
...
if __name__ == "__main__" :
    ...
    with open("MetroRentalFee.txt", 'w') as f:
        with ThreadPoolExecutor(max_workers=10) as executor:
            for station_id in range(1, 789): #다방 역 인덱스 1 - 788 까지 돌림
                executor.submit(record_station_data, station_id, f)
